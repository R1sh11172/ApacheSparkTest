...
//		csvData.show();
		...
		holdOutResults.show();
		holdOutResults = holdOutResults.drop("predction");
		
//		TrainValidationSplitModel model = trainValidationSplit.fit(trainingAndTestData);
//		LinearRegressionModel lrModel = (LinearRegressionModel) model.bestModel();
				
		
//		LinearRegressionModel model = new LinearRegression().setMaxIter(10).setRegParam(0.3).setElasticNetParam(0.8).fit(trainingData);
		...
		System.out.println("The training data r2 value is " + lrModel.evaluate(holdOutResults).r2() + " and the RMSE is " + lrModel.evaluate(holdOutResults).rootMeanSquaredError());
